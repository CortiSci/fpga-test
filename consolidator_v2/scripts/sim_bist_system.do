# Full-system four-Tail BIST: USB -> SPI_CFG -> Tail self-test -> V3 telemetry.
# The test drives only the FT600Q boundary.  RUN_BIST must never use force on
# DUT or inter-FPGA signals.

set USE_PLL_STUB 1
set DEFINES "+define+RUN_BIST+USE_PLL_STUB"
if {!$USE_PLL_STUB} { set DEFINES "+define+RUN_BIST" }
...
vsim -t 1ns -suppress 12110 -voptargs="+acc" -lib work -l "$SCRIPT_DIR/sim_bist_system.log" tb_top
run -all
